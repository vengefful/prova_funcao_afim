...
for aluno in alunos:
    print(f"Creating AVA student {aluno[1]}")
    aluno_dir = f"{aluno[1]} - {aluno[0]}"
    ...
    main_tex = re.sub(regex_ra, aluno[0], main_tex)
    main_tex = re.sub(regex_aluno, aluno[1], main_tex)
    ...
    merger.append(os.path.join(provas_dir, turma, aluno_dir, 'main.pdf'))

print("merging pdfs")
merger.write(f"{turma}.pdf")
merger.close()
print("Finished")